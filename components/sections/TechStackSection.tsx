...
const TechStackSection: React.FC = () => {
    return (
        <>
            <h1 className='text-3xl font-bold mb-4 text-gray-50 text-center md:text-left ml-4 md:ml-28'>TechStack</h1>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 gap-6 p-4 md:p-8 lg:px-24">
                {techs.map((tech) => (
                    <Card key={tech.name} className="flex items-center justify-center p-4 bg-customBlueGray rounded-lg shadow-md border border-slate-500 hover:border-blue-500 hover:-translate-y-2 transition-all duration-300 h-16">
                        <div className="text-3xl mr-2">{tech.icon}</div>
                        <h2 className="text-lg text-slate-200">{tech.name}</h2>
                    </Card>
                ))}
            </div>
        </>
